...
- Formally picking the production donor address and documenting rationale + per-session ceiling.
- Generalizing this pattern into `sandbox/scripts/utils/impersonation.ts` and replacing the `transferCoin(deployer → MM)` calls in `deploy-all.ts` (the DEEP funding-strategy plugin).
- Switching the faucet's DEEP path to the same mechanism.
...
The acceptance criterion for this spike is: the recipient ends up with a `Coin<DEEP>` at the real mainnet DEEP package ID equal to `TRANSFER_AMOUNT`, and the donor's balance drops by the same amount.
...
These data points unblock the DEEP funding-strategy plugin with concrete numbers to plan against.

## POC results

**Validated — empty-sig impersonation transfers real mainnet DEEP on the fork.** Run on 2026-06-04 against a `sui-fork` mainnet fork (`sui-fork`/`sui` both `1.74.0-31537d4d9235`).
...
**Surprises worth carrying into the DEEP funding-strategy plugin and the faucet wiring:**
...
4. **The whale holds DEEP under three package addresses** (`0xdeeb7a4…`, `0x4e164bf…`, `0xf91e818…`). Exact-type filtering is mandatory — production donor selection should pin the canonical mainnet DEEP type, not match on `::deep::DEEP`.
5. **Gas is negligible** (~1.45M MIST net) and is paid from the donor's own SUI, so the donor must hold SUI as well as DEEP — a constraint for production donor selection.
