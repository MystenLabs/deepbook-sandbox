...
- **Which discovered BM trades is a user choice.** Discovery returns every id; the page trades with `ids[0]` unless the user picks another from the selector in the Balance Manager card (rendered only when the wallet owns more than one). That preference — and only the preference, never the discovery — is persisted per wallet address in `localStorage` under `deepbook-sandbox:selected-bm`, shared across `useDeepBookClient` instances via a module store so the trading and market-maker pages agree. It exists because a manager's Bag entry can go permanently unreadable on the fork (SUI-FORK-ISSUES #10): that coin's deposits vanish, it reads 0, and every sell of it aborts with `abort code: 3` in `withdraw_with_proof` — undetectable by reading, so the only escape is switching managers.
